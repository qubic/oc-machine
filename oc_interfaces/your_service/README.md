...
2. Defines its `OcRequest` struct, byte-for-byte matching the Qubic Core interface header in
   `src/oc_interfaces/<Name>.h` (use `#pragma pack(push, 1)` and a `static_assert`
   on the size).
3. Returns its `interfaceIndex()` (matching `OC_INTERFACE_INDEX` in
   `src/oc_core/oc_interfaces_def.h`).
